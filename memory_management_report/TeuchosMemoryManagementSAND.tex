...
``view'' is some object that is created off of a parent object and
...
ToDo: Show a UML figure of a parent and a view.
...
of the parent objects that they are providing the views into (but not
always).  In the case of contiguous array containers like
{}\texttt{std::vector} and {}\texttt{ArrayRCP}, constraining the
implementation to store a pointer to a contiguous array of data
internally is not a problem.  However, for more general classes, the
rigid constraints imposed by direct views are unacceptable and break
the abstraction in most cases.  For example, if an abstract matrix
object provides direct views of the rows of a matrix then the matrix
must necessarliy be stored in a row-major data-structure, precluding
other possibilities.  Once a direct row-based view is supported by
such a matrix class, it becomes impossible to change the internal
representation of the matrix to anything other than a row-oriented
implementation.
...
a ``generalized view'' is needed is when you need to create a view of
...
new dense matrix.  This type of view is used in the implementation of
Thyra MultiVector non-contiguous column views [???] which is depecited
in Figure {}\ref{fig:MultiVectorView}. In the Thyra MultiVector
implementation, when the client requests a view of a set of
non-contiguous columns, the implemetion will create a temporary
contiguous copy which improves performance of many types of
operations.  Before describing the Thyra MultiVector example in more
detail below, first, we present a generic description of the
``generalized view'' idiom.

The most general statement of the ``generalized view'' idiom is Figure
...
\begin{center}
%\fbox{
\includegraphics*[angle=270,scale=0.65]{GeneralizedViewClassDiagram}
%}
\end{center}
\caption{
...
\begin{center}
%\fbox{
\includegraphics*[angle=270,scale=0.65]{GeneralizedViewStateDiagram}
%}
\end{center}
\caption{
