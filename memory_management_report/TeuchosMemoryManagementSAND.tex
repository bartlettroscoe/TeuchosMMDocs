...
%
{}\subsubsection{Limitations in debug-mode runtime checking}
%
...
in invalid C++ references.  Also, if you use references like in
Listing {}\ref{listing:raw-ref-dangling-ref}, then you can of course
...
Another situation where debug-mode runtime checking will not expose a
problem is when an object is not allocated on the heap with
{}\ttt{new} (or {}\ttt{new[]} for arrays) but an owning {}\ttt{RCP}
(or {}\ttt{ArrayRCP}) is created.  For example, consider the code in
...
The lack of debug-mode checking shown in Listing
{}\ref{listing:bad-delete-error} is unfortunate but it is very hard to
detect.  One way to avoid this problem is to make as many
reference-type classes as possible only allow creation through a
non-member constructor (see Section
{}\ref{sec:nonmember-constructor-idiom}) which always creates an
{}\ttt{RCP} right away and does not allow any other types of
allocation.  However, there will be value-type classes like
{}\ttt{std::vector} and {}\ttt{Teuchos::Array} that one will want or
need to dynamically allocate, hence leaving open this hole in the
debug-mode runtime checking.  The good news though is that memory
checking tools like Valgrind and Purify usually do a good job of
detecting and reporting erroneous calls to {}\ttt{delete} like this.
Perhaps in the future a portable library function can be written and
used that will be able to detect the difference between a stack
address and a heap address so an exception can be thrown right when
the bad owning {}\ttt{RCP} is first created.
