...
mainstream material on modern C++ usage in just the references [???,
???, ???] still results with a fundamentally unsafe language that
makes it too easy to write programs that segfault and leak memory.
...
background material or the many interesting side topic discussions.
Every reader should start with the introduction in Section ???.  For
experienced C++ programmers that just want to get started quickly
using the software, they should jump directly to Section ??? that
...
??? and ??? can be quickly browsed.  Section ??? should be read next
which gives the idioms for the use of the memory management classes.
If nothing else, the reader should at least look at the tables and
figures in this section which are also summarized in Appendix ???.
Finally, the material in Appendices ??? and ??? should be used as a
(relatively) short reference guide for the use of these classes.  This
material together with some existing code examples (e.g\ in Trilinos)
should give an experienced C++ develeloper enough to get started using
these classes in productive.  Finally, the reader should go through
...
% ToDo: Relace 'our' with more generic wording ... In Progress ...
...
type {}\ttt{B} it a base of type {}\ttt{A} is almost always asking for
...
exception handling mechanism using {}\ttt{try}/\ttt{catch} was clear
...
class; {}\ttt{auto\_ptr}.  Great detail about {}\ttt{auto\_ptr} is not
...
such as when {}\ttt{delete} is called.  Any form of language-supported
...
experience where I have hundreds of thousands of lines of C++ code on
...
data from {}\ttt{0} to {}\ttt{n-1}).  The utility classes in Teuchos
...
fragile and dangerous code because if {}\ttt{v} is resized, grown or
...
If one is unlucky, the statement {}\ttt{ele = 5} will work just fine
on one platform with one implementation of the {}\ttt{std::vector} but
...
to how many implementations of {}\ttt{std::vector} deal with memory.
...
Here, it is stated that {}\ttt{std::vector::operator[](size\_type)}
