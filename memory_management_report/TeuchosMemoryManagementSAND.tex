...
While the debug-mode runtime checking is of great importance, of equal
importance is performance in a optimized non-debug build.  It is
critical that the wise use of the Teuchos memory management classes
lead to optimized performance that is nearly identical to the
performance of raw pointers.  Otherwise, if there is alway a
performance gap with using the Teuchos memory management classes, then
there will always be an excuse to go back to using raw pointers will
all of the disasterous consequences discussed in Sections ??? and ???.
