...
{\small\begin{verbatim}
  ...
  public:
    ...
    RCP<A> get_A() { return a_; }
    RCP<B> get_B() { return b_; }
    RCP<C> get_C() { finalInitialization(); return c_; }
    void uninitialize() { a_ = null, b_ = null; c_ = null; }
    void doSomeOperation(...)
      {
        finalInitialization(); // Just-in-time initialization!
        ...
       }
  private:
    bool isIntialized_;
...
{}\texttt{finalInitialization()} can now call a virtual function on
...
{}\texttt{finalInitialization()} function is not called until the
...
{}\texttt{finalInitialization()} in every operation that requires the
...
{\small\begin{verbatim}
  ...
  public:
    ...
    RCP<VectorBase> createMember()
      {
         return defaultSpmdVector(selfRef_.create_strong());
       }
  private:
    RCP<DefaultSpmdVectorSpace> selfRef_;
    Ordinal localDim_;
...
{}\texttt{defaultSpmdVector( selfRef\_.create\_strong())}.  This
...
{\small\begin{verbatim}
  ...
  public:
    ...
    ...
  private:
    RCP<SomeClass> selfRef_;
    RCP<C> c_;
    SomeClass() {}
    void finalInitialization() { c_ = createC(selfRef_); }
...
{\small\begin{verbatim}
  ...
  {
    finalInitialization(); // Just-in-time initialization!
    return rcpWithEmbeddedObj(c_.getRawPtr(),
      std::make_pair(c_, selfRef_.create_strong()), false);
  }
\end{verbatim}}
\end{listing}
...
{}\texttt{RCP<SomeClass>} object is made null and will not throw.
Note that the use of {}\texttt{rcpWithEmbeddedObj(...)} shown in
Listing {}\ref{listing:osr:SomeClass-get_C-persisting} is an instance
of the ``inverted object ownership'' idiom which is described in
Section {}\ref{sec:inverting-obj-ownership}.

ToDo: Create the functions rcpInvertedObjOwnership(childRcp,
parentRcp) and getInvertedObjOwnershipParent<ParentT,
ChildT>(childRcp) and use this in all the examples.  This may be a
common use case.
...
Before describing the details of the ``generalized view'' idiom, first
a precise definition of a ``view'' is in order.  In this context, a
``view'' is some object that is created off of some parent object and
provides some type of access to some part of the the parent.  Views
can be changeable or non-changeable and can be persisting or
semi-persisting (see Section ??? for the definition of persisting and
semi-persisting associations).  Views can also be direct views or
potentially detached views. 

A direct view is one where the view directly points into the the
internal data structures of the parent so a change of the view
instantaneously changes the parent and changes to the parent changes
the view.  An exampmle of a direct view is an iterator into an
container such as is returned from {}\texttt{std::vector::begin()},
{}\texttt{Array::begin()} and {}\texttt{ArrayView::begin()}.  Other
examples of direct views include {}\texttt{ArrayView}s of
{}\texttt{Array} and {}\texttt{ArrayRCP} objects.  Direct views can be
changeable (non-const) and non-changeable (const) as is demonstrated
with iterators and {}\texttt{ArrayView}s.  Direct views are a plesure
to work with but they usually fundamentally break the encapsulation of
the parent objects that they are providing the views into (but not
always).  The behavior of a direct view object can significanly
constrain the possible implementations of the parent object.  For
example, if a matrix object provides direct views of the rows of a
matrix then the matrix must necessarliy be stored in a row-major
data-structure, precluding other possibilities.
...
instead consider using potentially detached non-direct views otherwise
defined here as a ``generalized view''.  A ``generalized view'' is a
...
propagated to the parent and visa versa.  One case where a
``generalized view'' is important is when you need to create a view of
non-contiguous columns of a dense matrix but for the sake of
efficiency you would like to create a temporary contiguous copy as a
new dense matrix.  This type of view is used in the implementation
Thyra MultiVector column views [???] and is depecited in Figure ??? .

ToDo: Add figure showing creating a contiguous copy of non-contiguous
columns.
