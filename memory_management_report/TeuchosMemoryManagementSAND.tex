...
types {}\ttt{Ptr}, and {}\ttt{RCP}, one has to use pointer syntax
using {}\ttt{*a} and {}\ttt{a->someMember()}.  The C++ language makes
...
C++0x) and {}\ttt{boost::array\_ptr}.  What is being suggested in this
...
the reference-counting overhead of {}\ttt{RCP}, then {}\ttt{Ptr} can
...
pointer in these cases.  At least with {}\ttt{Ptr}, invalid usage will
be checked for in a debug build so one does not loose any debug-mode
runtime checking when using {}\ttt{Ptr} instead of {}\ttt{RCP} if one
...
{}\ttt{unsigned int} {}\ttt{ptrdiff\_t} would limit one to an
