...
ToDo: Discuss the concept of a semi-persisting association with
iterators as the classic example.  We need to recognize that we need
to use semi-persisting associations some times for performance and
ease of programming.
...
ToDo: Move the ``Common nonmeber function'' items to a more general
...
ToDo: Change over to {}\texttt{createWidget()} as the default idiom
since it is more clear and state that {}\texttt{widget()} can also be
used in contexts where it is clear what is happening.  Also, change
all such non-member constructors to use the createSomeClass() name for
non-member constructors.
...
{}\texttt{ArrayRCP} should not be used to mange large collectiosn of
small arrays since the memory overhead can be significant.
...
or more on poor optimizing compiler (e.g.\ MSVC++ 2008).  Therefore,
if portable peformance of your application is critical, make sure and
use the reference-counting types at as high a level of granularity as
you can such that it does not damage the quality of the software
(i.e.\ flexibility, usability, maintainability).
...
relationships are not needed.  By using the types {}\texttt{Ptr} and
{}\texttt{ArrayView} instead of raw pointers we still get all of the
strong debug-mode runtime checking that is described in Section
{}\ref{sec:debug-mode-runtime-checking} (e.g.\ dangling reference
checking, null checking, range checking, etc.) yet removes all
...
Instead, we just need to set up the infrastructure to be able to
detect those types of invalid usage in a debug-mode build but yield
high performance an optimized build.  Therefore, it seems reasonble to
replace {}\texttt{ArrayRCP} in
...
\begin{listing}: Sparse matrix class interface bending the idioms on 
persisting relationships for the sake of performance \\
...
for persisting associations may yield acceptable performance.
...
fall back on the non-reference-counted types {}\texttt{Ptr} and
...
The ``separate construction and initialization'' idiom described here
is not specific to the use of the Teuchos memory management classes
but it does provide the basic foundation for the next idiom described,
the ``object self-reference'' idiom.  The set up the context for the
discussion, consider a typical design for a class shown in Listing
...
One of the most useful and powerful idioms related to the use of the
Teuchos memory management classes is the ``generalized view'' idiom.
In this context, a ``view'' is some object that is created off of a
parent object and provides some type of access to some part of the the
parent.  Views can be changeable or non-changeable and can be
persisting or semi-persisting (see Section
...
the view.  An exampmle of a direct view is an iterator into an
...
objects.  Direct views can be changeable (non-const) and
non-changeable (const) as is demonstrated with iterators and
{}\texttt{ArrayView}s.  Direct views are a plesure to work with but
they also fundamentally constrain the implementation of the parent
objects that they are providing the views into.  In the case of
contiguous array containers like {}\texttt{std::vector} and
...
that is an immportant and proper design constraint for these classes.
...
rows of a matrix then the matrix must necessarliy be stored in a
...
internal implementations of the parent and the view seprately, we must
...
non-contiguous columns of a dense matrix but for the sake of
efficiency one must create a temporary contiguous copy as a new dense
matrix\footnote{Using contiguous columns of a fortra-style
column-major dense matrix is require in order to take advantage of
high performance BLAS routines.}.  This type of generalized view is
used in the implementation of Thyra MultiVector non-contiguous column
views [???] which is depecited in Figure
...
the implemetion will create a temporary contiguous copy which results
in improved performance of many types of operations.  The Thyra
example of non-contiguous column views is a good example because it is
a simple case to describe yet has all the features needed to
demonstrate the workings and the generalized view idiom.
...
Parent and child classes for ``generalized view'' idiom .}
\end{figure}
\esinglespace}
...
State behavior for parent object in ``generalized view'' idiom .}
\end{figure}
\esinglespace}


Before describing the Thyra MultiVector example in more detail, first,
a generic description of the ``generalized view'' idiom is presented.
The most general description of the ``generalized view'' idiom is
shown in Figure {}\ref{fig:GeneralizedViewClassDiagram} (UML class
diagram) and Figure {}\ref{fig:GeneralizedViewStateDiagram} (UML state
diagram).  Figure {}\ref{fig:GeneralizedViewClassDiagram} shows two
generic classes, a {}\ttt{Parent} and a {}\texttt{View}.  In this
case, only one type of the view is shown but in reality there can be
several different types of views into a single parent object (as there
are in the Thyra MultiVector case).  A view is created using either
the {}\ttt{createNonconstView(...)} or the {}\ttt{createView(...)} 
functions.  In either case, the returned view is wrapped in {}\ttt{RCP}
object.  There are two purposes for warpping the view in an
{}\ttt{RCP} objects.  First, a new {}\ttt{View} object may need to be
created to satisify the view request (and therefore needs {}\ttt{RCP}
to take care to control the lifetime of the dynamically allocated
object).  Second, the action needed to resync the view up with the
parent can be attached to the customized deallocator of the
{}\ttt{View} object as an embedded object.  In fact, the
{}\ttt{Parent} object is not guaranteed to be updated after changes to
the view are made until the view is destroyed.  To demonstrate,
consider the client code in Listing
...
{\small\begin{verbatim}
  void changeParent(const Ptr<Parent> &parent)
  {
    ...
    // Destroy the view an resyc the view with the parent
    view = null
...
generalized view has been destroyed.  The state of the parent while a
changeable non-const view is active is undefined.  One of three
...
appropriate way.  The third option is somewhere inbetween the first
...
relatively strict usage protocal must be defined as shown in the
...
const views, and having a non-const view.  The default state of the
...
the state 'has const- views'.  In this state, the parent object is
...
const views are created is that creating a non-const view and then
...
data, the changes to the non-const view would not be expected to be
propogated to the existing const views.  Since this type of ambiguity
would destroy the abstraction, the generalized view idiom simply
...
views must also be disallowed.  In summary, when non-const views are
...
the parent must be locked down.  In essense, when a non-const view is
...
implemtations of the parent and the view in all cases, the generalized
view idiom states that views are semi-peristing; that is, the views
are only valid while the client's {}\texttt{RCP} to the parent is
still active and the view is not expected to live on past the lifetime
of the parent.  Therefore, any generalized views that remain after the
last strong {}\ttt{RCP<Parent>} object is released will result in
dangling reference views.  For example, the code in Listing
{}\ref{listing:generalized-view-dangling} would result in a dangling
reference exception in a debug-mode node-tracing build.
...
Another important aspect of the generalized view idiom is that it is
important to distinguish between changeable non-const views and
non-changeable const views.  In the case of const views, if a separate
copy of the data must be created to support the view, the data does
not have to be written back to the parent when the view is destroyed.
This makes const views fundamentally more efficient than non-const
views (which must write back their data when they are destroyed).
Because const views are potentially more efficient, the unadorned name
...
the parent object through the view, then the code will not compiler
and the developer will need to change to create a non-const view
through {}\ttt{createNonconstView(...)}.

Because the parent object gets locked down when generalized views are
active, it is important that client code only create views at the last
...
same satement where the view will be used as demonstrated in Listing
{}\ref{listing:generalized-view-min-lifetime}.


\begin{listing}:  Example of mimimizing the lifetime of a generalized
view  \\
...
The last issue to discuss related to the generalized view idiom is
that, depending on the nature of the parent and the view classes, it
may be reasonable to have the parent object partitioned into different
logical pieces and then apply the behaviors shown in Figure
{}\ref{fig:GeneralizedViewStateDiagram} to each of these logical
pieces separately.  For example, we can treat each row or each column
in a matrix object as a separate logical piece such that we can allow
separate views of each of the rows or columns independent of each
other.  This is the case with the Thyra MultiVector example to be
described below.  However, any bulk query operations on the parent
object (like taking an induced matrix norm) must be locked out while
non-const views are active.  Likewise, any bulk modifying operation
(like assigning all the matrix entries to zero) must be locked out
when any views are active.
...
generalized view and views can be appplied separately to different
logically distict parts of the parent object (e.g.\ to the rows or
columns of an abstract matrix object).

{}\item It is important to differentiate between changeable non-const
views and non-changeable const views.  While detached non-const views
must be copied back to parent when the view is released, const views
do not not need to be copied back when the view is released (therefore
improving performance).

{}\item It is critcial that {}\ttt{RCP} objects be used to wrap the
created view objects in order to allow the views to be dynamically
created and to allow for specialized copy-back behavior when non-const
views are destroyed.

{}\item The flexibility and perforamnce gains allowed by the
generalized view idiom come at the expense of more restricted usage
patterns of the parent and views.
  ...
  relates to the viewd part) while a non-const view is active.
  ...
  interface of the parent object (at least functonality that relates
...
ToDo: Write a Teuchos utiltiy class to implement the guts of the
debug-mode runtime checking needed to enforce the semantics of the
generalized view idiom.  Use this utility class in a mock Teuchos
object and in the Thyra::DefaultSpmdMultiVector class.

Now that a overview of the generalized view idiom has been given,
consider the Thyra MultiVector non-contiguous column view example
mentioned earlier.  A simplified class declaration for the Thyra
MultiVector subclass that implements non-contiguous views is shown in
...
Depiction of contiguous and non-continguous multi-vector column views.}
\end{figure}
\esinglespace}
