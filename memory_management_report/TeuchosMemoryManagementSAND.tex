...
using raw pointers, one can always use the types {}\ttt{Ptr},
{}\ttt{ArrayView} or {}\ttt{Array[RCP,View]::iterator} to yield raw
...
Here the templated classes {}\ttt{Ptr} and {}\ttt{RCP} are described
which are used to encapsulate raw C++ pointers to single objects in
more detail.  Again, {}\ttt{Ptr} is used for non-persisting
associations and {}\ttt{RCP} is for persisting associations.  Below,
and in all of the code listings, it is assumed that they are enclosed
in the {}\ttt{Teuchos} namespace or there are appropriate {}\ttt{using
Teuchos::XXX} declarations for the various names in place.
...
create them as views of {}\ttt{Array}, {}\ttt{ArrayRCP}, {}\ttt{Tuple}
and other {}\ttt{ArrayView} object.  If client code is routinely
creating {}\ttt{ArrayView} objects from raw pointers, then the code is
...
counting machinery that is always part of {}\ttt{ArrayRCP}, one may
not want to use {}\ttt{ArrayRCP} instead of {}\ttt{Array} but if the
overhead is not going to be significant, then going with
{}\ttt{ArrayRCP} instead of {}\ttt{Array} can be a good choice because
