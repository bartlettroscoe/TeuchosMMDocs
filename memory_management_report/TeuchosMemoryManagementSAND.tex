...
which the second illegal {}\ttt{RCPNode} is created (see
Section~\ref{sec:except-handling-debugging}).  A breakpoint can also
be set in the function {}\ttt{addNewRCPNode(...)} to see when the
other {}\ttt{RCPNode} object was created (see
Section~\ref{sec:except-handling-debugging}).
...
every exception message has a {}\ttt{Thrown number} associated with it
embedded in the error message of the exception object.  One can set a
conditional break-point in {}\ttt{TestForException\_break(...)} to
only stop when {}\ttt{throwNumber} has the right value.  For example,
if one needs to stop on {}\ttt{Throw number = 10}, then in GDB one can
set:
...
objects are created and destroyed.  On the {}\ttt{insertionNumber}
uniquely identifies a particular {}\ttt{RCPNode} object.  In builds
where there is no node tracing enabled, {}\ttt{insertionNumber} will
...
{}\ttt{justInTimeInitialize()} can now call a virtual function on
...
{}\ttt{justInTimeInitialize()} function is not called until the functions
...
to have a call the function {}\ttt{justInTimeInitialize()} in every
...
The memory function {}\ttt{createMember()} shown in
...
outlive the client's {}\ttt{RCP} references to the factory object
{}\ttt{DefaultSpmdVectorSpace}.  A simple example of this is shown in
