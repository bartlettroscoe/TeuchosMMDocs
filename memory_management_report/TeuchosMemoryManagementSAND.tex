...
codes.  The challenges of writing software that uses raw memory
management result in components that are overly ridgid in how they can
be used and reused which fundamentally detacts for the impact that
such software could otherwise have.  The problems created by the use
of raw memory management can single handily derail the vision of a
large interconnected network of reusable CS\&E software components
developed and used by many different CS\&E organizations [???].
...
levels in C++ has resulted in many negative several consequences in
the development of C++ software that other more modern languages
...
linger in the code for months or years and damage the most basic
...
to software written in other modern languages for the same abount of
effort.
...
is largely moving away from C++ to more modern languages that do not
require manual unchecked memory management.
