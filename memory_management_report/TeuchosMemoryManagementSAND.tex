...
classes is the typical reference-counted smart pointer class similar
...
experienced C++ developer enough to get started using these classes
...
in components that are overly rigid in how they can be used and
reused which fundamentally detracts from the impact that such software
...
like {}\ttt{boost::shared\_ptr} (which is the basis for the new C++0x
{}\ttt{std::shared\_ptr} class).  The Teuchos equivalent for these smart
pointer classes is {}\ttt{Teuchos::RCP} which is abbreviated here as just
{}\ttt{RCP} in sample code.  If the reader is not familiar with the basics of
...
detail, converting from a pointer for an array of type {}\ttt{B} to a
pointer of type of base type {}\ttt{A} is almost always asking for
disaster because the alignment of the base type {}\ttt{A} will be
wrong according to the full type {}\ttt{B} (again see {}\cite[Gotcha
...
where the full set of operations are not appropriate for both.  The
...
The introduction of {}\ttt{std::auto\_ptr} is the first example that I
...
user-controlled memory management facility using {}\ttt{new} and
{}\ttt{delete}.  There is a lot of C++ code out there that requires
that destructors for objects be called exactly when expected such as
when {}\ttt{delete} is called.  Any form of language-supported GC will
...
{}\ttt{std::vector} version of {}\ttt{addArrayIntoArray(...)} is
...
version of {}\ttt{addArrayIntiArray(...)}
...
{}\ttt{addArrayIntoArray(...)} makes the client code much cleaning and
...
of {}\ttt{malloc(...)} and {}\ttt{free(...)} and in inserting checks
...
Linux/gcc (before I learned the GCC had a checked STL
...
days of debugging was that {}\ttt{std::vector} used its own allocator
which allocated big chunks of memory through {}\ttt{malloc(...)}.  It
...
{}\ttt{malloc(...)}.  What the off-by-one error did was to write over
a library managed part of the memory block and that silent corruption
would doom a later attempt by {}\ttt{std::vector} to allocate memory.
...
the example code in the function {}\ttt{someBlockAlgo(...)} in Listing
{}\ref{listing:someBlockAlgo-std-vector-raw-ptr} which uses the raw
C++ pointer version of the function {}\ttt{addArrayIntoArray(...)} in
...
in {}\ttt{addArrayIntoArray(...)} in Listing
...
In case it is not obvious, the off-by-one error shown in Listing
...
the function {}\ttt{addArrayIntoArray(...)} can only operate on the
...
{}\ttt{int}, and {}\ttt{double} are value types.  Likewise, derived
class types like {}\ttt{std::complex} and {}\ttt{std::vector} are also
...
Note that the classes {}\ttt{A} and {}\ttt{B} remain unchanged because they do
not involve any persisting associations.  It is only the class {}\ttt{C} that
needed to be refactored to handle the persisting association with {}\ttt{B}.
...
association that can result in a segfault
...
code that is initializing the local reference {}\ttt{ele} and the
{}\ttt{std::vector} container object {}\ttt{v}.  This is very fragile and
dangerous code because if {}\ttt{v} is re-sized, grown or have some other type
of change, then the reference pointed to by {}\ttt{ele} can be invalid.  For
...
Therefore, functions like {}\ttt{std::vector::operator[](size\_type)} should
...
to be used in some special situations (but perhaps not directly used as
described below).
...
will always have to use raw C++ pointers (at least indirectly) in a non-debug
...
use the types {}\ttt{Ptr}, {}\ttt{ArrayView} or {}\ttt{ArrayType::iterator}
(where {}\ttt{ArrayType} is {}\ttt{Array}, {}\ttt{ArrayRCP}, or
{}\ttt{ArrayView}) to yield raw pointer performance in a non-debug optimized
...
The templated classes {}\ttt{Ptr} and {}\ttt{RCP} are described in more detail
in the next two sections which are used to encapsulate raw C++ pointers to
single objects.  Again, {}\ttt{Ptr} is used for non-persisting (and
semi-persisting) associations and {}\ttt{RCP} is for persisting associations.
Below, and in all of the code listings, it is assumed that such code is
enclosed in the {}\ttt{Teuchos} namespace or there are appropriate
{}\ttt{using Teuchos::XXX} declarations for the various names in place (as is
...
{}\ttt{RCP} objects owning the same reference-counted objects (Section
...
here.  However, some of the more advanced functionality for {}\ttt{RCP} that
...
references also not inherently unsafe as well?  After all, under the covers
...
involved, always use {}\ttt{Ptr} instead of a raw C++ reference (even if the
...
{}\item{}All of the array classes support a handy {}\ttt{getRawPtr()} function
that allows a client to get the base pointer address, or null, to the array.
The standard {}\ttt{std::vector} class supports no such function which is very
...
{}\ttt{ArrayView} and {}\ttt{ArrayRCP} and a little different than for
{}\ttt{Array} due to the different nature of these view classes as apposed to
the container class {}\ttt{Array}.  As described in Section
{}\ref{sec:teuchos-const-nonconst-pointer-objects}, the classes
{}\ttt{ArrayView} and {}\ttt{ArrayRCP} can encapsulate both non-const and
const types {}\ttt{T} as their template argument while {}\ttt{Array} can only
accept a non-const type {}\ttt{T}.  Therefore, the {}\ttt{std::vector}
compatible functions in {}\ttt{ArrayView} and {}\ttt{ArrayRCP} are all
{}\ttt{const} functions since they don't change what data these objects point
...
{}\item{}\ttt{ArrayView} in optimized mode has all trivial inlined functions
...
What makes {}\ttt{ArrayView} non-trivial and special, however, is that in a
...
The class {}\ttt{ArrayView} has no equivalent in boost or the current C++ or
...
can not develop such an effective type system without an integrated type like
{}\ttt{ArrayView}.
...
The class declaration for {}\ttt{Teuchos::ArrayRCP} is shown in Listing
...
{}\ttt{ArrayRCP} returns persisting subviews of data through the member
function {}\ttt{persistingView(...)}.  This means that the underlying array of
...
{}\ttt{ArrayRCP} can be used in place of {}\ttt{std::vector} (and therefore
{}\ttt{Array}) that only needs to size or resize the array in baulk and does
not need to flexibly grow or shrink the array.  It does this by supporting
functions like {}\ttt{assign(...)}, {}\ttt{resize(...)}, and {}\ttt{clear()}.
Because of the reference counting machinery that is always part of
{}\ttt{ArrayRCP} and support for all raw C++ pointer functionality (e.g.,
{}\ttt{ptr++}), one may not want to use {}\ttt{ArrayRCP} instead of
{}\ttt{Array} in may types of code.  However, if the overhead is not going to
be significant, then going with {}\ttt{ArrayRCP} instead of {}\ttt{Array} can
...
The class {}\ttt{ArrayRCP} does not attempt to replace {}\ttt{Array} but can
be a better choice in many cases where an {}\ttt{Array} may otherwise be used.

{}\ttt{ArrayRCP} supports explicit shallow conversion to {}\ttt{ArrayView}.
Requiring an explicit conversion from {}\ttt{ArrayRCP} to {}\ttt{ArrayView} in
consistent with the required explicit conversion from {}\ttt{RCP} to
{}\ttt{Ptr}.  As explained in Section
...
{}\ttt{ArrayRCP::operator()()} function is a very short-hand way to perform
this conversion.
...
{}\ttt{std::vector} that is integrated with the {}\ttt{ArrayView} class for
debug-mode runtime checking.  In an optimized build, {}\ttt{Array} is nothing
but an inline wrapper around a fully encapsulated {}\ttt{std::vector} object.
This means that in an optimized build, {}\ttt{Array} takes advantage of all of
the platform-specific optimizations contained in the native
{}\ttt{std::vector} implementation and imparts no extra space/time overhead
...
detection of {}\ttt{ArrayView} views or direct conversions to {}\ttt{ArrayRCP}
objects (see Section {}\ref{sec:detection-dangling-references}).
{}\ttt{Array} also supports better runtime debug output with better exception
error messages.
...
{}\ttt{tuple(...)} are provided to make it easy to pass in arrays to
functions that accept them as {}\ttt{ArrayView} arguments.  Overloads
of {}\ttt{tuple(...)} are currently provided from one up through 15
arguments.  For an example for using {}\ttt{tuple(...)} to call a
...
{}\ttt{ArrayView} views are shown in Table
{}\ref{tbl:common-array-type-members}.  The {}\ttt{ArrayRCP} class can
...
refactored version to use {}\ttt{ArrayView} is shown in Listings
...
consider the case where the {}\ttt{addArrayIntoArray(...)}  function
...
{}\ttt{ArrayRCP}.  As explained in Section
...
implicit conversion for {}\ttt{T*} to {}\ttt{const T*} for
...
pointer types for {}\ttt{T*} to {}\ttt{const T*} and for {}\ttt{Base*}
to {}\ttt{Derived*} (or combinations of both with {}\ttt{Derived*} to
...
will be allowed to satisfy the function call.
...
{}\textit{non-member constructor} compliant {}\ttt{Widget} class
...
the {}\ttt{Array<RCP<A> >} object itself such as shown in Listing
{}\ref{listing:change-ref-type-objs-not-array}:
...
case {}\ttt{const Ptr<RCP<A> >\&} is shown in the tables which is a
...
associations are always passed out as {}\ttt{Ptr} and
...
not allow raw pointers so one must pass a {}\ttt{const Ptr<T>\&}
...
{}\ttt{outArg(...)} templated non-member function that returns a
{}\ttt{Ptr<T>} object given a raw reference {}\ttt{T\&}.
...
Theoretically, the compile-time check would appear to be far superiors
...
{}\ttt{Ptr} to a C++ object (for a non-persisting association) but the
function's implementation must create (and destroy) and object what
has a persisting association with the passed in object.  Consider the
...
node look-up as described in Section
{}\ref{sec:detection-dangling-references}).
...
expression {}\ttt{flab\_ = flab()}.  When this happens, the object now
represented as the raw C++ reference {}\ttt{flab\_in} is deleted which
causes the code in the expression {}\ttt{flab\_in->conflab(*flab\_)}
...
now two instead of one, the expression {}\ttt{flab\_ = flab()} will
not delete the existing {}\ttt{Flab} object and the following
...
{}\ref{listing:ref-count-mem-funcs}.  Most of these functions are never
called by general clients except in desperate situations.  Notable
exceptions are the member functions {}\ttt{create\_weak()} (which is
used to create a {}\ttt{WEAK} {}\ttt{RCP} object from a {}\ttt{STRONG}
object) and and {}\ttt{create\_strong()} (which is used to create a
{}\ttt{STRONG} {}\ttt{RCP} object from a {}\ttt{WEAK} object).  The
function {}\ttt{create\_weak()} is used to break a circular reference
as described in Section {}\ref{sec:circular-references-weak-pointers}
while {}\ttt{create\_strong()} is used in situations like the ``object
...
{}\ttt{C} through different base class interfaces.  If the programmer
...
through a {}\ttt{strength} attribute with values {}\ttt{STRONG} and
...
{}\ttt{clientB}) the objects {}\ttt{a} and {}\ttt{b} will go away also
...
(i.e.\ {}\ttt{deleteClientAFirst=true}) depicted in Figure
...
first (i.e.\ {}\ttt{deleteClientAFirst=false}) depicted in Figure
...
{}\ttt{weak\_ptr}, this is impossible (at least not without storing
both smart pointer types in each class {}\ttt{A} and {}\ttt{B} object
...
runtime approach of the {}\ttt{RCP} class (and therefore also the
{}\ttt{ArrayRCP} class) is well worth this small extra overhead.
Typically, the classes {}\ttt{RCP} and {}\ttt{ArrayRCP} are used to
manage objects (or blocks of array data for {}\ttt{ArrayRCP}) much
...
who use the classes {}\ttt{RCP} and {}\ttt{ArrayRCP} will never need
...
{}\ttt{malloc(..)} and {}\ttt{free(...)} through C library defined
...
{}\ttt{RCP} (which are identical for {}\ttt{ArrayRCP} as well) are
...
all of the objects of type {}\ttt{A} are created and removed
seamlessly without the client code that interacts with {}\ttt{RCP} and
...
embedding objects in the {}\ttt{RCPNodeImpl} object is that it can be
...
deallocator class {}\ttt{EmbeddedObjDealloc} shown in Listing
...
{}\ttt{DeallocDelete} (see Listing {}\ref{listing:RCP-dealloc}) and
set by the non-member constructor functions in Listing
{}\ref{listing:RCP-embedded-obj}.  The distinction between pre- and
...
therefore the client would just use {}\ttt{rcpWithEmbeddedObj(...)} to
...
wants to be able to use the reference count on {}\ttt{RCP} objects
...
for the embedded object at the time when the first {}\ttt{RCP} object
is created.
...
to template the {}\ttt{RCPNodeImpl} class also needs to be known.  If
...
one has to use the function {}\ttt{createRCPWithBadDealloc()} to
allocate {}\ttt{A} objects and are stuck with a pre-compiled library
and one cannot access the source code to fix the problem.  On most
...
reference-counted object {}\ttt{A} through a passed-in
reference-counted class object of type {}\ttt{RCP<A>}.
...
{}\ref{sec:nonmember-constructor-idiom}.  When the first {}\ttt{RCP}
...
The default behavior, of course, is to just simply call {}\ttt{delete}
(or {}\ttt{delete []} for arrays) on the contained raw pointer.
...
{}\ttt{ArrayRCP} for arrays) as described in Section
...
this to checked STL implementations that may nor may not exist on a
...
{}\ttt{RCPNode} objects that are still remaining, then a error message
is printed to {}\ttt{std::cerr} that gives all the details of the
objects involved in the circular reference.
...
tracing is enabled, the program ends an error message like the
...
example of why it is so important to use {}\ttt{Array} instead of raw
{}\ttt{std::vector} objects.  Strong debug-mode runtime checking of
{}\ttt{ArrayView} views are not possible when using
{}\ttt{std::vector}.
...
non-owning {}\ttt{RCP} reference {}\ttt{a\_rcp2} in Listing
...
or {}\ttt{ArrayRCP} object (see Section
...
{}\ttt{std::vector} is used instead of {}\ttt{Array} in Listing
...
elements (at {}\ttt{5.84-06 sec}) as shown in Appendix
...
fragments for the {}\ttt{Array} class are shown in Listing
...
However, just because one can not use {}\ttt{RCP} and {}\ttt{ArrayRCP}
...
{}\ttt{ArrayRCP} in {}\ttt{SparseMatrix::getSparseRow(...)} in Listing
{}\ref{listing:SparseMatrix-ArrayRCP} with {}\ttt{ArrayView} yielding
the new {}\ttt{SparseMatrix} interface shown in Listing
...
{}\ref{listing:SparseMatrix-dangling-ref} where the client code tries
...
The third problem with the design of {}\ttt{SomeClass} shown in Listing
{}\ref{listing:sci:SomeClass-before} is that the {}\ttt{C} object that is
...
The memory function {}\ttt{createMember()} shown in Listing
{}\ref{listing:osr:DefaultSpmdVectorSpace} creates a strong owning {}\ttt{RCP}
...
{}\ttt{DefaultSpmdVectorSpace}.  A simple example of this is shown in Listing
...
{}\ref{listing:osr:DefaultSpmdVectorSpace-use-delete} may seem contrived but
...
idiom is useful.  For one such case, consider Listing
{}\ref{listing:osr:SomeClass-before} which shows a simplified version of the
...
The problem with the code in Listing {}\ref{listing:osr:bad-use-SomeClass} is
that when the {}\ttt{SomeClass} object {}\ttt{sc} is destroyed, there is no
way for the reference-counting machinery in to catch the dangling reference.
This code will segfault if one is lucky but like any memory usage error, if
one is unlucky the code will appear to work correctly but will be a ticking
time-bomb that will go off eventually.  The reason for this behavior is that
the statement {}\ttt{c\_ = createC(rcp(this, false))} in the constructor
{}\ttt{SomeClass()} creates a non-owing {}\ttt{RCPNode} object
{}\underline{before} the owning {}\ttt{RCPNode} object is created by the
client code {}\ttt{RCP<SomeClass> sc(new SomeClass)}.  This violates
Commandment {}\ref{cmnd:owning-rcp-first} in Section
{}\ref{apdx:commandments}.  The node-tracing reference-counting machinery
would have to be {}\underline{much} more complex and expensive to catch
dangling reference errors where the strong owning {}\ttt{RCPNode} object was
not the first {}\ttt{RCPNode} object created.
...
dangling-reference exception is thrown as described in Section
{}\ref{sec:detection-dangling-references}.
...
{}\cite{ThyraOperatorVectorSAND} which is depicted in Figure
...
demonstrate, consider the client code in Listing
{}\ref{listing:generalized-view-ex1}.
...
However, while there are active const views, a non-const view can not
be created.  The reason that non-const views can not be allowed while
...
client's code.  For example, the code in Listing
{}\ref{listing:generalized-view-dangling} is most likely an error in
...
imposed by the generalized view design pattern shown in Figure
{}\ref{fig:GeneralizedViewStateDiagram} ensure that no problems will
...
specific key advantages of the {}\ttt{RCP} class that can not be
...
a) circular references (Section
{}\ref{sec:detection-circular-references}) and b) multiple owning
...
object (the extremely rare need for this is described in Section
{}\ref{sec:extra-data}).
...
low compared to raw allocations and deallocations (see Section
{}\ref{sec:reference-counting-overhead}) there is little reason to
...
{}\ttt{std::vector} object it can not detect a dangling reference (see
...
situation is that a large existing code-base must continued to be
developed , current code must be modified, and new code must
...
replacing the use of raw pointers and raw calls to {}\ttt{new} and
{}\ttt{delete} along the way.
...
required object is represented as {}\ttt{1} (see Figure
...
guidelines) that if followed, along with the idioms defined in Section
{}\ref{sec:idioms}, then client code will be nearly 100\% safe through
debug-mode runtime checking and will almost never result in a segfault
or other memory usage error (except for circular references as
described in Section {}\ref{sec:detection-circular-references}).
...
objects for shared all persisting associations (see Section
{}\ref{sec:nonpersisting-persisting-associations}).
\end{commandment}
...
and explicit conversion functions (see Section
{}\ref{sec:conversions}).  Thou shall never expose a raw C++ pointer
to perform a conversion.
\end{commandment}
{}\textit{Exception}: Some very advanced and rare use cases might have
one exposing a raw C++ pointer (see Section
...
in C/C++ programing (see Section
{}\ref{sec:essentail-accidental-complexity}).
...
{}\ttt{char} array 75\% the size of the maximum allowed by
