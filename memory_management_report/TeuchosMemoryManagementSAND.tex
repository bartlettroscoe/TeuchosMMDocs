...
{\small\begin{verbatim}
  ...
  private:
    ...
    RCP<C> c_;
    SomeClass() {}
    void justInTimeInitialization() { c_ = createC(selfRef_); }
...
{\small\begin{verbatim}
  ...
  {
    justInTimeInitialization();
    return rcpInvertedObjOwnership(c, selfRef_.create_strong());
  }
\end{verbatim}}
\end{listing}
...
Note that the use of {}\texttt{rcpInvertedObjOwnership(...)} shown in
...
Section {}\ref{sec:inverting-obj-ownership}.  It is required to create
a strong {}\texttt{RCP} in this case in order to properly set up the
true persisting association.
...
direct views or potentially detached non-direct views (i.e.\
``generalized views'').
...
always).  For example, if a matrix object provides direct views of the
...
anything other than a row-oriented implementation.  In the case of
contiguous array containers, constraining the implementation to be a
contiguous array of data internally is not a problem.  However, for
more general classes, the rigid constraints imposed by direct views is
unacceptable and breaks the abstraction in most cases.
...
embedded with a B object (inverting the ownership relationship) \\
