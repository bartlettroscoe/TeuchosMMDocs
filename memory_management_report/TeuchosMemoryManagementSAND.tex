...
Rather than being a radical approach this is simply the logical
progression of a trend in the C++ development and standards community
that started with {}\ttt{std::auto\_ptr} and is continued (but not
finished) with {}\ttt{std::shared\_ptr} in C++0x.
...
management smart-pointer types.}
\end{table}
...
a\_ptr}; {}\ttt{RCP<A> a\_rcp}; {}\ttt{ArrayView<S> s\_av};
{}\ttt{ArrayRCP<S> a\_arcp}; {}\ttt{Array<S> s\_a}; {}\ttt{Tuple<S,N>
...
DR = debug-mode Dangling Reference detection \\

{}\textcolor{red}{Warning: Conversions to and from raw pointers shown
in {}\underline{red} are strongly discouraged and should only be
performed in the special cases discussed in Appendix
{}\ref{apdx:commandments} and Section
{}\ref{sec:role-of-raw-pointers}.} \\

{}\textcolor{magenta}{Warning: Conversions from raw pointers to owning
{}\ttt{RCP} and {}\ttt{ArrayRCP} objects shown in
{}\underline{magenta} are stricly necessary but must be performed with
great caution according to the commandments in Appendix
{}\ref{apdx:commandments}.} \\
...
{}\ttt{Ptr} described in Section {}\ref{sec:Ptr}

\end{enumerate}

\end{minipage}
%
\caption{\label{tbl:AllConversions}
Summary of the basic conversions that are supported involving the
Teuchos memory management classes.}
%
\end{center}
%
...
ToDo: Provide a table of all the types of conversions that aresupported (implicit or explicit) and give the function that performs
