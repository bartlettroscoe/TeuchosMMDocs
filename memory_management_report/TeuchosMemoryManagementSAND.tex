...
back, a philosophical discussion of the tradeoffs between speed,
...
constructors, and assignemnt operators (all implementing deap copy
semantics),
...
but usaully carries greater significance in implying that operator
...
embedd these types in each other in any arbitrary order to create any
type of data structure that you would like.  For example, you could
have {}\texttt{Array<RCP<ArrayRCP<ArrayView<Tuple<Ptr<T>,5> > > > >
>}.  By undestanding what each of these types provide and what each
...
the allowed upper and lower bounds.  It supports all the approriate
...
{}\texttt{clear()}.  Because of the reference counting machinary that
...
{}\texttt{Array} provide a complete catelog of all the debug-mode
...
As soon as a continguous array of data is correctly captured in
{}\texttt{Array} or and owning {}\texttt{ArrayRCP} object, all
children {}\texttt{ArrayView} objects will be protected in that if the
parent array gets seleted, a debug-mode runtime check will detect and
...
{}\texttt{std::vector} but resulted in verbose clumbsy code that was
...
safetly, let's consider the case where the
...
If the erroreous {}\texttt{addArrayIntoArray(...)} function in Listing
...
for specific examples).  It is crtical that the programmer recognize
...
reinterpret casts for contingous arrays of POD (plain old data). It is
prefectly reasonable to allocate a large array of {}\texttt{char}
