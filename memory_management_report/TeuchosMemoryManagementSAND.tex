...
have.  The material in Appendices {}\ref{apdx:summary_of_idioms} and
...
experienced C++ developer enough to get started using these classes in
a productive way.  Finally, the reader should go through the full
table of contents at least once to get an idea of the variety of
topics covered and where to look when more information is needed (and
most developers will need to know most of this extra information at
some point).
...
persisting versus non-persisting associations are defined in Section
...
perhaps the first section one would jump to in order to get a quick
idea what the Teuchos memory management classes are all about.
Finally, taking a step back, the concepts of essential and accidental
complexity and a philosophical discussion of the trade-offs between
speed, safety and generality related to memory management are
discussed in Section~\ref{sec:misc-topics}.  Concluding remarks are
...
Note that virtual funtions can not be templated so it is not possible
...
{}\texttt{transform(...)} function in Listing
...
functions like {}\ttt{transform(...)} in Listing
...
pointer version of {}\ttt{transform(...)} in Listing
...
accessors to the underlying raw C++ pointer member {}\ttt{ptr\_}.
...
of raw C++ references is exploited (as explained in Section
{}\ref{sec:raw-C++-references}) in defining idioms that increase the
...
{}\ttt{ArrayView} and {}\ttt{ArrayRCP} are a little different than for
...
{}\ref{listing:someBlockAlgo-std-vector-raw-ptr} but in addition also have full
debug-mode runtime error checking.  To see the improved safety, let's
consider the case where the {}\ttt{transform(...)}  function
...
If the erroneous {}\ttt{transform(...)} function in Listing
...
element.  As mentioned in Section
...
high-level C++ code, but it also means that raw calls to {}\ttt{new}
...
expression {}\ttt{flab\_ = createFlab()}.  When this happens, the object now
...
now two instead of one, the expression {}\ttt{flab\_ = createFlab()} will
...
lack of debug-mode checking shown in Listing
{}\ref{listing:bad-delete-error} is unfortunate but it is very hard to
...
created.  }.  Note that the code in this example violates Commandment
...
violates Commandment {}\ref{cmnd:owning-rcp-first} in Section
...
not constant reference (see Section
{}\ref{sec:idioms-returning-objects}).
\end{commandment}
