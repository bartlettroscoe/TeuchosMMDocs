...
raw pointers.  Otherwise, if there is alway a performance gap with
using the Teuchos memory management classes, then there will always be
an excuse to go back to using raw pointers will all of the disasterous
consequences discussed in Sections ??? and ???.
...
ToDo: Show table giving the details of the three platforms a) gcc ??? 
on Linux, b) Intel ??? on Linux, c) MS Visual C++ ??? on MS Windows
Vista.
...
tests were run on three different platforms with three different
comilers shown in Table ??? that represent a reasonable range of the
main-stream platforms.  Timing results on other compilers may give
different results, especially for compilers that have a bad history at
optimizing C++ code (e.g.\ PGI, Sun, AIX etc.).  All of these
performance timing tests are driven by a performance testing framework
in Teuchos and there are nightly tests that strictly enforce relative
...
braket operator (i.e.\ {}\texttt{a[i]}) and iterator (i.e.\
...
accurate timings, one must perform the operaton in loop and average
...
loops sizes are given in Appendix {}\ref{apdx:raw-perf-data}.

ToDo: Show a chart showing the relative times for allocation and
destruction for different size arrays for all three compilers.  This
will be a line chart with three lines, one for each compiler.
...
ToDo: Show a bar chart for the relative times for each operation for
each compiler.  This should be be a bar chart showing all three
compilers.
