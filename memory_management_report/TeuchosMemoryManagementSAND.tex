...
template<class Dealloc_T, class T> const Dealloc_T& get_dealloc(const RCP<T>& p);
template<class Dealloc_T, class T> Dealloc_T& get_nonconst_dealloc(const RCP<T>& p);
...
{}\cite{RefCountPtrBeginnersGuide}.  Some of the more advanced
functionality for {}\texttt{RCP} is described in later chapters that
is not described in {}\cite{RefCountPtrBeginnersGuide}.
...
{\small\begin{verbatim}
  ...
  public:
    ...
    // Common array members (see above) and other functions
    ...
...
{}\textit{accedential complexity} {}\cite{MythicalManMonth95}.
{}\textit{Accedential complexity} in programming refers to complexity
...
Accedental complexity has largely been removed as higher level
langages have been developed {}\cite[Chapter 16]{MythicalManMonth95}.
...
lingering category of accedental
...
nature of the problem at hand and no programming lanague will remove
...
accedental complexity of using raw pointers and manual resource
...
that are implicit in most lanaguages (including raw C++).  Dealing
...
other end of the association?).  In UML class diagrams, a singlular
...
user-defined object is always managed through an indrect reference
...
Section ???], the author trys to determine if the passed in object is
...
vs.\ non-persistent, changable vs.\ non-changeable) are present in
...
period.  One (of many) of the remaining types of accedental complexity
in C++ is the distiction between value semantics verses reference
...
you to remove unecessary constraints in the use of objects in removing
arbitrary lifetime ordering constraints which are a type of unecessary
coupling {}\cite{CodeComplete2nd04}.  The code you write with these
classes will be more likely to be correct on first writing, will be
less likely to contain silent (but deadly) memory usage errors, and
will be much more robust to later refactoring and maintainance.  These
classes and idioms largely address the technical issues in resolving
the fragile built-in C++ memory management model (with the exception
of circular references which has no easy solution but can be managed
as discussed).  All that remains is to teach these classes and idioms
and expand their usage in C++ codes.  The long-term viability of C++
as a usageable and productive language depends on it.  Otherwise, if
C++ is no safer than C, then why bother with C++?  C is smaller and
easier to learn and since most programmers don't know object-oriention
all that well anyway then what are they really getting out of C++
other than more complexity?  C++ zelots will argue this point but the
reality is that C++ is becoming less popular while the popularity of C
is remaining failry stable over the last 10 years.
