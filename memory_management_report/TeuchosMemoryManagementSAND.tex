...
performed by the C++ compiler to satify the formal arguments for a
...
(or a tempate function where all of the template arguments are
...
language usability annoyance associated with templates and has nothing
specific to do with the Teuchos memory management classes themselves.)
...
When user code trys to call {}\ttt{someUserFunction(...)} as shown in
...
occuring, one must understand the C++ type system in how it handles
basic type conversions.  The C++ standard specifies that implicit type
conversions to facilitate the call of a C++ funtion will only occur
...
non-const reference instead of by value, the complier will refuse the
